...
void find_labels(FILE *fptr)
{
	...
	while (fgets(buffer, MAX_LINE_LENGTH, fptr)) {
		...
		if (debug) printf("[DEBUG] LABEL %s @ line %u\n",
...
int main(int argc, char **argv)
{
	...
	while (fgets(buffer, MAX_LINE_LENGTH, fptr)) {
		...
		if (buffer[0] == '\0') continue;  // skip empty lines and comments
		if (debug) printf("[DEBUG] \"%s\"\n", buffer);
		if (bfrcmp(buffer, "PUSH")) push(get_push_number(buffer));
		else if (bfrcmp(buffer, "PULL")) pull();
		else if (bfrcmp(buffer, "DUP")) dup();
		else if (bfrcmp(buffer, "ADD")) add();
		else if (bfrcmp(buffer, "SUB")) sub();
		else if (bfrcmp(buffer, "MUL")) mul();
		else if (bfrcmp(buffer, "DVD")) dvd();
		else if (bfrcmp(buffer, "SAY")) say();
		else if (bfrcmp(buffer, "END")) end();
		else if (bfrcmp(buffer, "JUMP_IF_ZERO")) 
					jump_if_zero(fptr, buffer, line);
		else if (bfrcmp(buffer, "JUMP")) jump(fptr, buffer, line); 
		else if (bfrcmp(buffer, "STORE")) 
			store(get_store_number(buffer)); 
		else if (bfrcmp(buffer, "LOAD")) 
			load(get_load_number(buffer));  
		else if (bfrcmp(buffer, "SWAP")) swap();
		else if (bfrcmp(buffer, "MOD")) mod();
		/*to prevent unknown instruction errors*/	
		else if (bfrcmp(buffer, "LABEL")) goto skip_error;

		else fprintf(stderr, "Unknown instruction at line %d: %s\n",
							line, buffer);
		skip_error: ;
		if (show_machine) {
			printf(" ------- \n");
